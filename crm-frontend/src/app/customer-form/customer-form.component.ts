...
export class CustomerFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.id != -1)
      this.service.retrieveCustomer(DEFAULT_USERNAME, this.id).subscribe(
  ...
  onSave() {
      if(this.id == -1) {
        ...
        this.service.addCustomer(DEFAULT_USERNAME, this.customer).subscribe(
          response => {
        ...
        this.service.updateCustomer(DEFAULT_USERNAME, this.customer).subscribe(
          response => {
            console.log(response);
            this.router.navigate(['customers']);
          }
